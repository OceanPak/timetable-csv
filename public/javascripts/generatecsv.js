module.exports = function(timetable, cdates) {
  ...
  var masterstr = '';
  masterstr += 'Subject,Start Date,Start Time,End Date,End Time,Location,Description\n';
  
  for (i = 0; i < splitday.length; i++) {
    var day = splitday[i];
    
    var dates = cdates[day[0].split(' ')[1].slice(0, 2)];
    ...
    for (j = 0; j < dates.length; j++) {
      for (k = 0; k < classes.length; k++) {
        ...
        if (c != undefined) {
          ...
          var s = subject + ',' + startDate + ',' + startTime + ',' + endDate + ',' + endTime + ',' + location + ',' + description + '\n';
          masterstr += s;
        }
      }
    }
